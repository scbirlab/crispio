...
def _load_genome_and_gff(genome: TextIOWrapper, 
                         gff: TextIOWrapper) -> Tuple[FastaSequence, GffFile]:

    for fasta_sequence in FastaCollection.from_file(genome).sequences:
        break

    fasta_sequence = replace(fasta_sequence,
    ...
    new_metadata = [('genome-sequence', 'constrained', [fasta_sequence.name, 1, len(fasta_sequence.sequence)]), 
...
def _prepare_to_search(args: Namespace) -> Tuple[FastaSequence, GffFile, str, Dict[str, Union[str, int]]]:
    ...
    fasta_sequence, gff_data = _load_genome_and_gff(args.genome,
                                                    args.annotations)
    ...
    sgRNA_defaults = dict(seqid=fasta_sequence.name,
                          source=__package__,
                          feature='protospacer',
                          score='.',
                          phase='.')
...
def _map(args: Namespace) -> None:
    ...
    guide_library = GuideLibrary.from_mapping(guide_seq=guide_sequences,
                                              genome=fasta_sequence.sequence,
                                              pam_search=pam_search)

    for guide_match in guide_library.as_gff(max=1,
...
def _generate(args: Namespace) -> None:
    ...
    guide_library = GuideLibrary.from_generating(max_length=args.max_length,
...
def _featurize(args: Namespace) -> None:
    ...
    with tqdm(list(input_gff.lines)) as t:  ## run a progress bar
        ...
        for gff_line in t:
            ...
            new_features = featurize(gff_line, 
                                     scaffold=scaffold)
...
def _offtarget(args: Namespace) -> None:
    ...
    new_metadata = [('crosstalk-comparator', 'free', [os.path.basename(args.gff2.name)])]
    old_metadata = list(gff1.metadata.data)
    new_metadata = replace(gff1.metadata, 
    ...
    with tqdm(gff1.lines) as t:
        ...
        for gff_line1 in t:
            ...
            for gff_line2 in gff2_lines:
                ...
                pair, added_mm = _get_mismatches(gff_line1, gff_line2, 
...
def main():
    ...
    length_max = CLIOption('--max_length', '-l', 
                           type=int, 
                           default=20,
                           help='Maximum length.')
    length_min = CLIOption('--min_length', '-m', 
                           type=int, 
                           default=None,
                           help='Minimum length.')
    
    # featurize
    scaffold = CLIOption('--scaffold', '-s', 
                        dest='scaffold', 
                        type=str, 
                        required=True,
                        help=('Name of a scaffold from "{}" or '
                              'a scaffold sequence.').format('", "'.join(sequences.scaffolds)))
    
    # offtarget
    gff2 = CLIOption('--gff2', '-2', 
                     type=FileType('r'),
                     required=True,
                     help='GFF file containing protospacers for comparison.')
    mismatches = CLIOption('--mismatches', '-e', 
                           type=int, 
                           default=2,
    ...
    genome = CLIOption('--genome', '-g', 
                       type=FileType('r'),
                       required=True,
                       help='Genome FASTA file')
    annotations = CLIOption('--annotations', '-a', 
                            type=FileType('r'),
                            required=True,
                            help='GFF file.')
    attributes = CLIOption('--attributes', '-t', 
                            type=str, 
                            nargs='*',
